# 2D-DriftPlane-Turbulence-Te-Ti
...
h_avg = (h('+') + h('-'))/2
# The potential equation terms (interior penalty formulation)
potential_terms = (
    inner(grad(phi_s), grad(v_phi)) * dx  # Laplacian term for phi (CG part)
    ...
    # Boundary terms (assuming homogeneous Dirichlet BCs)
    - dot(grad(v_phi), p_h_s * normal) * ds  # Consistency
    - dot(v_phi * normal, grad(p_h_s)) * ds  # Symmetry
    + (alpha/h) * v_phi * p_h_s * ds  # Penalty
    
    + w_s * v_phi * dx  # Source term
)
...
output_file = VTKFile("./2D-DriftPlane-Turbulence-Te-Ti/2D-DriftPlane-Turbulence-Te-Ti_output.pvd")
