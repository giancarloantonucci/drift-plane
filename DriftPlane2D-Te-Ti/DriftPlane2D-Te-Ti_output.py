# DriftPlane2D-Te-Ti_output.py
# DriftPlane2D-Te-Ti using DG advection / CG elliptic solve
...
V_w = FunctionSpace(mesh, "DQ", 4, type=="spectral")  # for w (vorticity)
V_n = FunctionSpace(mesh, "DQ", 4, type=="spectral")  # for n (electron density)
V3 = FunctionSpace(mesh, "DQ", 4, type=="spectral")  # for pe (electron pressure)
V4 = FunctionSpace(mesh, "DQ", 4, type=="spectral")  # for ph (ion pressure)
# V = V_w * V_n
V = V_w * V_n * V3 * V4
...
# VTKFile("DriftPlane2D-Te-Ti_output_initial.pvd").write(solution.sub(0), solution.sub(1), solution.sub(2), solution.sub(3))
...
F = (
    ...
    + driftvel_n('+') * ( n('+') - n('-') ) * v_n('+') * dS
    + Dt(pe) * v3 * dx
    - v3 * div(pe * driftvel) * dx
    + Constant(0.1) * v3 * pe * dx
    + Dt(ph) * v4 * dx
    - v4 * div(ph * driftvel) * dx
    + Constant(0.1) * v4 * ph * dx
    + driftvel_n('-') * ( pe('-') - pe('+') ) * v3('-') * dS
    + driftvel_n('+') * ( pe('+') - pe('-') ) * v3('+') * dS
    + driftvel_n('-') * ( ph('-') - ph('+') ) * v4('-') * dS
    + driftvel_n('+') * ( ph('+') - ph('-') ) * v4('+') * dS
)
...
# Main loop
output_file = VTKFile("DriftPlane2D-Te-Ti_output.pvd")
...
VTKFile("DriftPlane2D-Te-Ti_output_final.pvd").write(solution.sub(0), solution.sub(1), solution.sub(2), solution.sub(3), phi_s)
